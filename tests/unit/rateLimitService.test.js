...
describe('rateLimitService', () => {
  ...
  it('throttles requests when limit is exceeded', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await checkRateLimit(ip, { limit: 3, windowMs: 60_000 })
    }

    const blocked = await checkRateLimit(ip, { limit: 3, windowMs: 60_000 })
    expect(blocked.isAllowed).toBe(false)
    expect(blocked.remaining).toBe(0)
  })

  it('resets quota after windowMs has elapsed', async () => {
    vi.useFakeTimers()
    ...
    const fresh = await checkRateLimit(ip, { limit: 2, windowMs: 1000 })
    expect(fresh.isAllowed).toBe(true)
    expect(fresh.remaining).toBe(1)
  })

  it('applyRateLimit sets HTTP headers and sends 429 response when throttled', async () => {
    const req = {
    ...
    expect(await applyRateLimit(req, res, { limit: 2, windowMs: 60_000 })).toBe(true)
    expect(await applyRateLimit(req, res, { limit: 2, windowMs: 60_000 })).toBe(true)

    // 3rd call -> should be throttled
    const isAllowed = await applyRateLimit(req, res, { limit: 2, windowMs: 60_000 })
