...
export const NavBar = ({ useStore }) => {
  ...
  const mobileNavItems = () => (
    <div className="d-lg-none mt-3 pt-3 border-top border-secondary border-opacity-25">
      {session ? (
        ...
        <Button variant="primary" size="lg" className="w-100" onClick={() => setShowLoginModal(true)}>
  ...
  const desktopLoginOrProfile = () => {
    if (session) {
      ...
      return <div className="d-none d-lg-block"><ProfileDropdown useStore={useStore} /></div>;
    }
  }
  ...
  return (
    <Navbar expand="lg" className="bg-body-tertiary bg-dark" data-bs-theme="dark">
      ...
      <LoginForm show={showLoginModal} onHide={() => setShowLoginModal(false)} useStore={useStore} />
    </Navbar>
  )
}
